...
class DoubleDQN(nn.Module):
    def __init__(self, env, tau=0.05, gamma=0.9, epsilon=1.0):
        ...
        self.target_model = DQN(self.obs_shape, self.action_shape, self.encoder).to(
            DEVICE
        )
            ...
            for episode in range(EPISODES):
                ...
                for step in range(STEPS):
                    ...
                    transition = Transition(
                        state, action, goal, reward, next_state, [], done
                    )
                ...
                for step in range(len(trajectory)):  #
                    ...
                    deltas = future_instruction_relabeling_strategy(
                        trajectory, step, 4, 0.9
                    )
                    for delta in deltas:
