...
class Transition:
    def __init__(
        self, current_state, action, goal, reward, next_state, satisfied_goals_t, done
    ):
...
def get_state_based_representation(observation, ghat, f1_model):
    ...
    for i in range(observation.shape[0]):
        for j in range(observation.shape[1]):
            for k in range(observation.shape[1]):
                data.append(
                    torch.cat((observation[i, j, :], observation[i, k, :]), 0).to(
                        DEVICE
                    )
                )
    output = f1_model(torch.stack(data))
    Z_matrix = output.view(
        observation.shape[0], observation.shape[1], observation.shape[1], -1
    )
    ...
    w_matrix = torch.stack(
        [
            torch.dot(z_vec, ghat[idx])
            for idx, batch in enumerate(Z_matrix)
            for row in batch
            for z_vec in row
        ]
    )
    ...
    zhat = torch.stack(
        [
            torch.stack([torch.sum(torch.stack(rows)) for rows in batch])
            for batch in z_vector
        ]
    )
